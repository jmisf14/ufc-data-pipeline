from ufcstats_scrape.items import Event, Fight, Fighter
...
class UfcstatsScrapePipeline:
    def __init__(self):
        ...
        self.cursor.execute(
            open("ufcstats_scrape/sql/create_events_table.sql", "r").read())
        self.cursor.execute(
            open("ufcstats_scrape/sql/create_fights_table.sql", "r").read())
        self.cursor.execute(
            open("ufcstats_scrape/sql/create_fighters_table.sql", "r").read()
        )
    ...
    def process_item(self, item, spider):
        if isinstance(item, Event):
            ...
            self.cursor.execute(
                open("ufcstats_scrape/sql/select_fighters.sql", "r").read(),
                (item.id,))
            if (
    ...
    def close_spider(self, spider):
        ...
        try:
            if self.events:
                psycopg2.extras.execute_batch(
                    self.cursor,
                    open("ufcstats_scrape/sql/insert_into_events.sql", "r").read(),
                    list([(event.id,
                           event.name,
                           event.date,
                           event.location,
        ...
        try:
            if self.fights:
                psycopg2.extras.execute_batch(
                    self.cursor,
                    open("ufcstats_scrape/sql/insert_into_fights.sql", "r").read(),
                    list([(
                        fight.id,
                        fight.event_id,
                        fight.red_id,
        ...
        try:
            if self.new_fighters:
                psycopg2.extras.execute_batch(
                    self.cursor,
                    open("ufcstats_scrape/sql/insert_into_fighters.sql", "r").read(),
                    list([(
                        fighter.id,
                        fighter.first_name,
                        fighter.last_name,
        ...
        try:
            if self.existing_fighters:
                psycopg2.extras.execute_batch(
                    self.cursor,
                    open('ufcstats_scrape/sql/update_fighters.sql', 'r').read(),
                    list([(
                        fighter.first_name,
                        fighter.last_name,
                        fighter.t_wins,
