...
class UpcomingPipeline:
    def __init__(self):
        ...
        self.cursor.execute(
            open("stat_scrape/sql/create_upcoming_events_table.sql", "r").read()
        )
        self.cursor.execute(
            open("stat_scrape/sql/create_upcoming_fights_table.sql", "r").read()
        )
    ...
    def close_spider(self, spider):
        ...
        try:
            if self.upcoming_events:
                psycopg2.extras.execute_batch(
                    self.cursor,
                    open("stat_scrape/sql/upsert_upcoming_events.sql", "r").read(),
                    [
                        (
                            event.id,
                            event.name,
        ...
        try:
            if self.upcoming_fights:
                psycopg2.extras.execute_batch(
                    self.cursor,
                    open("stat_scrape/sql/insert_upcoming_fights.sql", "r").read(),
                    [
                        (
                            fight.event_id,
                            fight.red_name,
